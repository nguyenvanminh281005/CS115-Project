...
class Sampling(layers.Layer):
    ...
    def __init__(self, **kwargs):
        ...
        self.seed = 1337  # Lưu seed dưới dạng thuộc tính

    def call(self, inputs):
        z_mean, z_log_var = inputs
        batch = tf.shape(z_mean)[0]
        dim = tf.shape(z_mean)[1]
        epsilon = tf.random.normal(shape=(batch, dim), seed=self.seed)  # Sử dụng tf.random.normal
...
vae.fit(mnist_digits, epochs=70, batch_size=128)
